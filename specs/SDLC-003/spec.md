...
`docs/antipattern-guard-spec.md` specifies a second plugin — Antipattern Guard — that installs
...
- Marketplace and validation support for a second plugin in `plugins/`, with independent
  versioning.
- A stated hook-ownership boundary between `sdlc`'s `guard` and Antipattern Guard, with the
...
Out:
- Implementing Antipattern Guard itself — `docs/antipattern-guard-spec.md` is its spec and
  `docs/plans/2026-09-16-antipattern-guard.md` is its plan.
...
- `docs/antipattern-guard-spec.md` §7.1, §7.2, §4.3, §12 (D9).
...
| UC-4 | maintainer | `make bump-apguard VERSION=0.2.0` | only the second plugin's `plugin.json` and its `marketplace.json` entry change | integration |
